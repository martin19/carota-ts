...
export class RunBase {
  ...
  constructor(text:string|Array<string>, formatting : IFormattingMap) {
    this.text = text;
    this.formatting = RunBase.cloneFormatting(formatting);
  }
  ...
  static sameFormatting(run1:RunBase, run2:RunBase) {
  ...
  clone() {
    return new RunBase(this.text, this.formatting);
  }

  static merge(run1:RunBase|Array<RunBase>, run2?:RunBase):RunBase {
    if (arguments.length === 1) {
      return Array.isArray(run1) ? run1.reduce(RunBase.merge) : run1;
    }
    if (arguments.length > 2) {
      return this.merge(Array.prototype.slice.call(arguments, 0));
    }
    if(run1 instanceof RunBase && run2 instanceof RunBase) {
      ...
      this.formattingKeys.forEach(function (key) {
        if (key in run1.formatting || key in run2.formatting) {
          if ((run1.formatting)[key] === (run2.formatting)[key]) {
            ...
            mergedFormatting[key] = RunBase.multipleValues;
          }
        }
      });
    }
  ...
  format(template:IFormattingMap) {
    Object.keys(template).forEach(function (key) {
      if ((template)[key] !== RunBase.multipleValues) {
        this.formatting[key] = template[key];
      }
    });
  }

  static format(run:RunBase|Array<RunBase>, template:IFormattingMap) {
    if (Array.isArray(run)) {
      run.forEach((r:RunBase)=>{
   ...
   * @returns {function(function(RunBase): void, RunBase): void}
   */
  static consolidate() {
    var current:RunBase;
    return (emit:(p:RunBase)=>void, run:RunBase)=> {
  ...
  static getPlainText(run:RunBase) {
